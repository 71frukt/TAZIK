...
void ReverseFrontInit(Node *init_node, FILE *dest_file)
{
    ...
    if (init_node->left->type == KEY_WORD && init_node->left->val.key_word->name == VAR_T_INDICATOR && init_node->right != NULL)
        fprintf(dest_file, "%s ", KeyWords[ASSIGN].my_symbol);
...
void ReverseFrontNewExpr(Node *new_expr_node, FILE *dest_file)
{
    ...
    if (new_expr_node->left->type != KEY_WORD || (new_expr_node->left->val.key_word->name != IF && new_expr_node->left->val.key_word->name != WHILE))  // для if и while эта нода искусственная, её в исходном коде нет
        fprintf(dest_file, "%s\n", KeyWords[NEW_EXPR].my_symbol);
...
void ReverseFrontendComma(Node *cur_comma, FILE *dest_file)
{
    ...
    {
        fprintf(dest_file, "%s ", KeyWords[COMMA].my_symbol);
...
{
    fprintf(dest_file, "%s ", KeyWords[IF].my_symbol);
